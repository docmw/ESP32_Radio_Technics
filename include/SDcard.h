...
#define MAXSPACE    15000                             // Max space for filenames (bytes, not tracks).
...
struct mp3spec_t                                      // For List of mp3 file on SD card
{
  ...
  #define SDSPEED 40000000                              // SPI speed of SD card
  ...
  void getsdtracks ( const char * dirname, uint8_t levels )
  {
    ...
    while ( file )
    {
      ...
      else                                                // It is a file
      {
        ...
        if ( ( strcmp ( ext, ".MP3" ) == 0 ) ||           // It is a file, but is it an audiofile?
            ( strcmp ( ext, ".mp3" ) == 0 ) || ( strcmp ( ext, "flac" ) == 0 ) || ( strcmp ( ext, ".ogg" ) == 0 ))
        {
  ...
  void handle_ID3_SD ( String &path )
  {
    ...
    mp3file = SD_MMC.open ( path ) ;                              // Open the file
  ...
  bool mount_SDCARD ( int8_t csPin )
  {
    ...
    if ( !SD_MMC.begin ("/sdcard", true) )             // Yes, try to init SD card driver
    {
       dbgprint ( "SD Card Mount Failed!" ) ;             // No success, check formatting (FAT)
    }
    else
    {
       SD_okay = ( SD_MMC.cardType() != CARD_NONE ) ;         // See if known card
       if ( !SD_okay )
       {
