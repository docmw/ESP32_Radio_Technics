...
//Technics remote
#define t_power 218152113
#define t_play 218124381
#define t_pause 218124381
#define t_back 218141343
#define t_stop 218103821
#define t_foward 218124895
#define t_top menu 218159572
#define t_display 218122564
#define t_menu 86536002
#define t_up 218145196
#define t_down 218128748
#define t_left 218161644
#define t_right 218108188
#define t_enter 218120524
#define t_1 218105861
#define t_2 218138757
#define t_3 577270721
#define t_4 278230305
#define t_5 218114085
#define t_6 218146981
#define t_7 218130533
#define t_8 218163429
#define t_9 218109973
#define t_0 218142869
#define t_10 218141084
#define t_tvpower 16825533
#define t_tv 83950076
#define t_md 83937740
#define t_TVvoldown 16811141
#define t_TVvolup 16778245
#define t_mute 83905609
#define t_tv_av 497446431
#define t_pageup 86523152
#define t_pagedown 86556048
...
#define QSIZ              200                         // Number of entries in the queue
...
#define MEDIASERVER_DEFAULT_MAC "B8:27:EB:8C:0B:CC"
#define MEDIASERVER_DEFAULT_IP 192,168,0,200
#define MEDIASERVER_DEFAULT_PORT 1900
#define MEDIASERVER_DEFAULT_CONTROL_URL "music"

#define LC7218_REG_REFERENCE_12_5_KHZ ((uint8_t)0x02)
#define LC7218_REG_REFERENCE_10_KHZ ((uint8_t)0x01)
#define LC7218_IF_FREQUENCY_FM ((uint16_t) 10700u)
#define LC7218_IF_FREQUENCY_AM ((uint16_t) 450u)
#define LC7218_MONO_OUTPUT ((uint8_t)1u << 2)
#define LC7218_CS_PIN 12
#define LC7218_CLK_PIN 27
#define LC7218_DATA_PIN 26

#define MAX_DELAY_RDS 80  //  polling method
#define MAX_DELAY_STATUS 5000
#define MAX_DELAY_SHOW_RDS 80

#define STATION_WITH_RDS_SERVICE 9600  // Local station with good RDS service (Example: 89,90Mhz)
...
struct button_t {
  const uint8_t pin;
  uint32_t last_time_pressed;
  bool pressed;
};

enum fm_lc7218_state_t 
{
  FM_IDLE,
  FM_IN_PROGRESS,
  FM_DATA_READY
...
struct fm_reg_t {
  ...
  uint8_t selection_sc:1;
  uint8_t switching_sf:1;
  uint8_t selection_gt:1;
  uint8_t timebase:1;
  uint8_t lsi:2;
};
...
struct fm_status_t {
  ...
  uint16_t signal_voltage;
  fm_lc7218_state_t lc7218_state;
  bool is_stereo;
  bool set_mono;
  bool fm;
  bool tuned;
...
#endif
enum enum_selection { NONE, STATION, SDPLAYER, MEDIASERVER }; // play mode or selected source
...
void        FM_Decode_LC7218_reg(fm_reg_t *);
void        FM_Decode_Data(fm_reg_t *);
void        FM_Data_Read_ISR();
void        RemoteProcess();
...
//FM radio
fm_reg_t    LC7218_Reg;
volatile uint64_t    LC7218_Raw_Data;
fm_status_t FM_Input_Status;
uint32_t    last_fm;
...
bool blockIfButtonRepeat = false;
unsigned long lastTime = millis();
unsigned long lastResult = 0;


enum player_mode_t { RADIO, MP3} ;              // State for player
player_mode_t        player_mode = RADIO ;               // Default is RADIO mode
uint8_t filebuffer[2048];

#ifdef ENABLE_SOAP
SoapESP32         soap(mp3client2, NULL);                // UDP/SSDP not used, no global SPI lock 
...
void setup()
{
  ...
  {
    SPI.begin ( 18,                   // Init VSPI bus with default or modified pins
                19,
                23 ) ;
  }
  ...
  dataqueue = xQueueCreate ( QSIZ,                       // Create queue for communication
    ...
    VS1053_begin ( 5,                  // Make instance of player and initialize
                   16,
                   21,
...
if ( NetworkFound )
 ...
 // disableCore1WDT() ;
  ...
  FM_Input_Status.current_frequency = 88000u;
  FM_Input_Status.last_frequency = 88000u;
  FM_Input_Status.lc7218_state = FM_IDLE;
  FM_Input_Status.new_data = false;
  FM_Input_Status.tuned = false;
  pinMode(LC7218_CS_PIN, INPUT_PULLDOWN);
  pinMode(LC7218_CLK_PIN, INPUT_PULLDOWN);
  pinMode(LC7218_DATA_PIN, INPUT_PULLDOWN);
  attachInterrupt((digitalPinToInterrupt(LC7218_CLK_PIN)), FM_Data_Read_ISR, RISING);
  rx.setup();
    ...
    if(rx.getRdsReady())
      {
        if(rx.hasRdsInfo())
        {
          ...
          stationName = rx.getRdsText2B();
      
          rdsMsg2 = rx.getRdsText0A();
          return;          
        }
...
void loop()
{
  ...
  digitalWrite(15, LOW);
  if((FM_DATA_READY == FM_Input_Status.lc7218_state) && (true == FM_Input_Status.new_data))
  {
      FM_Decode_Data(&LC7218_Reg);
      FM_Decode_LC7218_reg(&LC7218_Reg);
      FM_Input_Status.new_data = false;
      FM_Input_Status.lc7218_state = FM_IDLE;
      dbgprint ("New FM data") ;
      dbgprint("divider %d", LC7218_Reg.pdivider);
      dbgprint("rawdata %d", LC7218_Raw_Data);
  }
  if((millis() - last_fm > 500) && (FM_IN_PROGRESS == FM_Input_Status.lc7218_state))
  {
    FM_Input_Status.lc7218_state = FM_IDLE;
  }
  if(FM_Input_Status.current_frequency != FM_Input_Status.last_frequency)
  {
      dbgprint("New FM frequency: %d KHz", FM_Input_Status.current_frequency);
      rx.clearRdsBuffer();
      rx.setFrequency((FM_Input_Status.current_frequency / 10));
      FM_Input_Status.last_frequency = FM_Input_Status.current_frequency;
  }

  if ((millis() - rds_elapsed) > 100) {
    showRDS();
    rds_elapsed = millis();
  ...
  if ((millis() - status_elapsed) > MAX_DELAY_STATUS) {
    ...
    if ( rdsMsg2 != NULL ) Serial.println(rdsMsg2);
...
const char* analyzeCmd ( const char* par, const char* val )
{
  ...
  {
    uint16_t mode = vs1053player->read_register(0x9);
    sprintf ( reply, "Free memory is %d, chunks in queue %d, bitrate %d kbps, mode: %04X",
              heapspace,
...
void sdfuncs()
{
  ...
  if ( openfile )
  {
    ...
    {
      if ( uxQueueSpacesAvailable ( dataqueue ) >= ((sizeof(filebuffer)) / 32) )                   // Space in queue?
      {
        size_t n = read_SDCARD ( filebuffer,
                                 sizeof(filebuffer) );
        uint16_t current = 0;

        while(current < n) 
        {
          for(uint8_t i = 0; i<sizeof(outchunk.buf); i++)
          {
            ...
            if(current >= n) {
              ...
              outchunk.buf[i] = filebuffer[current];
              current++;
            }          
          }
          xQueueSend ( dataqueue, &outchunk, 0 ) ;
        }
                          // Send to queue
...
void FM_Decode_LC7218_reg(fm_reg_t * fm_reg)
{
    uint16_t reference_freq;
    uint16_t divider = 0u;
    uint8_t i;

    if((LC7218_REG_REFERENCE_12_5_KHZ == fm_reg->reference) && 0x01u == (uint8_t)fm_reg->divider)
    {
      FM_Input_Status.fm = true;
      reference_freq = 25;
      divider = (fm_reg->pdivider) & (0xFFFE) ;
      FM_Input_Status.current_frequency = (uint32_t)(((uint32_t)divider * reference_freq) - LC7218_IF_FREQUENCY_FM + (uint32_t)50u);
    }
    else
    {
      FM_Input_Status.fm = false;
      reference_freq = 10;
      divider = (((fm_reg->pdivider) >> 4)) & (0x0FFF);
      FM_Input_Status.current_frequency = (uint32_t)(((uint32_t)divider * reference_freq) - LC7218_IF_FREQUENCY_AM);
    }

    Serial.print("New_frequency: ");
    Serial.print(FM_Input_Status.current_frequency);
    Serial.println("Khz");
    if((uint8_t)0u == (fm_reg->output & LC7218_MONO_OUTPUT))
    {
      FM_Input_Status.set_mono = true;
    }
    FM_Input_Status.new_data = false;
    FM_Input_Status.lc7218_state = FM_IDLE;
}

void FM_Decode_Data(fm_reg_t * fm_reg)
{
  fm_reg->pdivider = (uint16_t)(LC7218_Raw_Data);
  fm_reg->output = (uint8_t)(LC7218_Raw_Data >> 16u);
  fm_reg->cten = (uint8_t)(LC7218_Raw_Data >> 23u);
  fm_reg->reference = (uint8_t)(LC7218_Raw_Data >> 24u);
  fm_reg->divider = (uint8_t)(LC7218_Raw_Data >> 28u);
  fm_reg->sensitivity = (uint8_t)(LC7218_Raw_Data >> 29u);
  fm_reg->selection_sc = (uint8_t)(LC7218_Raw_Data >> 30u);
  fm_reg->switching_sf = (uint8_t)(LC7218_Raw_Data >> 31u);
  fm_reg->selection_gt = (uint8_t)(LC7218_Raw_Data >> 32u);
  fm_reg->timebase = (uint8_t)(LC7218_Raw_Data >> 33u);
  fm_reg->lsi = (uint8_t)(LC7218_Raw_Data >> 34u);
}
...
void FM_Data_Read_ISR()
{
  ...
  if(HIGH == digitalRead(LC7218_CS_PIN))
  {
    last_fm = millis();
    if(FM_Input_Status.lc7218_state != FM_IN_PROGRESS)
    {
      FM_Input_Status.lc7218_state = FM_IN_PROGRESS;
      FM_Input_Status.new_data = false;
      current_bit = 0u;
      LC7218_Raw_Data = (uint64_t)0u;      
    }
    if(HIGH == digitalRead(LC7218_DATA_PIN))
    {
      LC7218_Raw_Data |= ((uint64_t)1u << current_bit);
    }
    current_bit++;
    if(36u == current_bit)
    {
      FM_Input_Status.lc7218_state = FM_DATA_READY;
      FM_Input_Status.new_data = true;
      current_bit = 0u;
    }
...
void RemoteProcess() {
  if (irrecv.decode(&results)) {
    ...
    if (millis() - lastTime > 200) {
      blockIfButtonRepeat = false;
    }
    
    
    if (results.value == lastResult && blockIfButtonRepeat) {
      ...
      switch (results.value) {
        ...
        case t_down:
          ...
          break;
        case t_left:
          analyzeCmd ( "downpreset", "1" );
          break;
        case t_right:
          analyzeCmd ( "uppreset", "1" );
          break;
        case t_enter:
